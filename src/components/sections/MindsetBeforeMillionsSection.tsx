...
interface MindsetBeforeMillionsSectionProps {
  showOrderNow?: boolean;
}

export function MindsetBeforeMillionsSection({
  showOrderNow = true
}: MindsetBeforeMillionsSectionProps): JSX.Element {
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto grid w-full max-w-[980px] grid-cols-1 items-center gap-10 lg:grid-cols-[1fr_380px] lg:gap-14">
          <motion.div
            className="order-1 flex flex-col items-start gap-8"
          ...
          <motion.div
            className="order-2 mx-auto w-full max-w-[330px] sm:max-w-[360px] lg:max-w-[380px]"
            initial={{ opacity: 0, x: 24 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, amount: 0.35 }}
